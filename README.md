...
The step runs:
```sh
dnf check-update --secseverity=Critical
```

Based on its exit code (please read more at [Check-Update Command](https://dnf.readthedocs.io/en/latest/command_ref.html#check-update-command-label)), determines if there any packages' *Critical* updates are available, and for scheduled workflow continue or not with image(s) building.
...
In  dispatch mode, image(s) are built regardless of the presence of *Critical* updates.
